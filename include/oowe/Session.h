...
class Session
{
    ...
    public:
        ...
        String escape  (char * str, int len);
        String escape  (const std::string & str);
        String escape  (const String & str);
        String unescape(char * str, int len);
        ...
        char *       getEffectiveUrl         (void); // Get the last used effective URL
        ...
        char *       getRedirectUrl          (void); // Get the URL a redirect would take you to if you would enable FOLLOWLOCATION
        ...
        char *       getContentType          (void); // Get the content-type of the downloaded object
        ...
        char *       getPrimaryIp            (void); // Get the string holding the IP address of the most recent connection done with this session
        long         getPrimaryPort          (void); // Get the destination port of the most recent connection done with this session
        char *       getLocalIp              (void); // Get the pointer to a zero-terminated string holding the local (source) IP address of the most recent connection done with this session
        ...
        char *       getFtpEntryPath         (void); // Get a string holding the path of the entry path
...
//        TLSSESSION   getTlsSession           (void); // Get an enum indicating the SSL library used for the handshake and the respective internal TLS session structure of this underlying SSL library
        ...
        char *       getRtspSessionId        (void); // Get a string holding the most recent RTSP Session ID
...
//        void setErrorBuffer(char * buffer); // Set error message buffer
        ...
        // Network options
        void setUrl                (char * url);          // Set URL to work on
        ...
        void setUnixSocketPath     (char * path);         // Set path to a Unix domain socket

        // Authentication options
        void setNetrc              (long   level);    // Set enable .netrc parsing
        void setNetrcFile          (char * file);     // Set .netrc file name
        void setUserPassword       (char * userpwd);  // Set user name and password
        void setProxyUserPassword  (char * userpwd);  // Set proxy user name and password
        void setUsername           (char * username); // Set user name
        void setPassword           (char * pwd);      // Set password
        void setLoginOptions       (char * options);  // Set login options
        void setProxyUsername      (char * username); // Set proxy user name
        void setProxyPassword      (char * pwd);      // Set proxy password
        void setHttpAuth           (long   auth);     // Set HTTP server authentication methods [HTTP]
        void setTlsUsername        (char * username); // Set TLS authentication user name
        void setTlsPassword        (char * pwd);      // Set TLS authentication password
        ...
        void setOAuth2BearerToken  (char * token);    // Set OAuth2 bearer token
        ...
        void setDnsInterface        (char *   ifname);     // Set bind name resolves to this interface
        void setDnsLocalIp4         (char *   address);    // Set bind name resolves to this IP4 address
        void setDnsLocalIp6         (char *   address);    // Set bind name resolves to this IP6 address
        void setDnsServers          (char *   servers);    // Set preferred DNS servers
        void setAcceptTimeoutMillis (long     ms);         // Set timeout for waiting for the server's connect back to be accepted

        // SSL & Security options
        void setSslCertificate                   (char * cert   ); // Set client cert
        void setSslCertificateType               (char * type   ); // Set client cert type
        void setSslPrivateKeyFilename            (char * keyfile); // Set client key
        void setSslPrivateKeyType                (char * type   ); // Set client key type
        void setKeyPassword                      (char * pwd    ); // Set client key password
        void setSslEnableAlpn                    (bool   enable ); // Set enable use of ALPN [HTTP]
        void setSslEnableNpn                     (bool   enable ); // Set enable use of NPN [HTTP]
        void setSslEngine                        (char * id     ); // Set use identifier with SSL engine
        ...
        void setCaInfo                           (char * path   ); // Set CA cert bundle
        void setSslIssuerCertificateFilename     (char * file   ); // Set issuer certificate
        void setCaPath                           (char * path   ); // Set path to CA cert bundle
        void setCertificateRevocationListFilename(char * file   ); // Set certificate Revocation List
        void setCertificateInfo                  (bool   enable ); // Set extract certificate info
        void setPinnedPublicKey                  (char * key    ); // Set set pinned SSL public key 
        void setRandomFile                       (char * path   ); // Set provide source for entropy random data
        void setEgdSocketPath                    (char * path   ); // Set identify EGD socket for entropy
        void setSslCipherList                    (char * list   ); // Set ciphers to use
        ...
        void setKerberosSecurityLevel            (char * level  ); // Set kerberos security level [FTP]
